...
class LUT_OT_Export(bpy.types.Operator, ExportHelper):
    ...
    baseDisplay: bpy.props.EnumProperty(name="Reference Display", items=listDisplays, default=0)
    baseView: bpy.props.EnumProperty(name="Reference View Transform", items=listViews, default=0)
...
class LUT_OT_Import(bpy.types.Operator, ImportHelper):
    """ Imports the LUT """
    bl_idname = "sequencer.lut_impport"
    bl_label = "Import LUT"
    ...
    def LUTcompositorGroup(self, context, texture, name, size):
        ...
        connect(inputNode, multiplyCoordsNode, 0, 0, nodeGroup)
